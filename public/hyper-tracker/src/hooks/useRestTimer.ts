...
export function useRestTimer(totalDefault: number, onDone?: () => void, ownerId?: string) {
  ...
  const setRestActive = useStore((s) => s.setRestActive);
  ...
  // keep store in sync (who owns the current REST state)
  useEffect(() => {
    setRestActive(active, ownerId ?? null);
  }, [active, ownerId, setRestActive]);
